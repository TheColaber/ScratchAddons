...
export default async function ({ addon, global, console, msg }) {
  ...
  const triggerTooltip = {
    ...
    studios(element) {
      ...
      let tippyOptions = {
        ...
        onShow(instance) {
          ...
          instance._isFetching = true;
          fetch(`https://api.scratch.mit.edu/studios/${id}`)
            .then((response) => response.json())
            .then((data) => {
              let text1 = document.createElement("span");
    ...
    users(element) {
      ...
      let tippyOptions = {
        ...
        onShow(instance) {
          ...
          instance._isFetching = true;
          fetch(`https://api.scratch.mit.edu/users/${id}`)
            .then((response) => response.json())
            .then((data) => {
              let text1 = document.createElement("span");
    ...
    async () => {
      while (true) {
        const element = await addon.tab.waitForElement("a[href^='https://scratch.mit.edu/projects/']", {
          markAsSeen: true,
        });
    ...
    while (true) {
      const element = await addon.tab.waitForElement("a[href^='https://scratch.mit.edu/studios/']", {
        markAsSeen: true,
      });
