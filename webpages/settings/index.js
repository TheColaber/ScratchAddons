  ...
  for (const { manifest, addonId } of manifests) {
    ...
    for (let i = 0; i < keyInputs.length; i++) {
      keyInputs[i].addEventListener("keydown", function (e) {
        e.preventDefault()
        e.target.value = (e.ctrlKey ? "Ctrl" + (e.shiftKey ? " + Shift" : "") + (e.key == "Control" || e.key == "Shift" ? "" : (e.ctrlKey ? " + " : "") + e.key.toUpperCase()) : "")
      ...
      keyInputs[i].addEventListener("keyup", function (e) {
        e.target.value = e.target.value == "Ctrl" || (objectVals.filter((v) => (v === e.target.value)).length > 1) ? "" : e.target.value
      })
    }
  }, 2000);
